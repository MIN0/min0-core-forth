...
> [!TIP]
> `WORDS`の中に気になる名前を見つけたら、**[ワード・ポケットリファレンス](WORD_REFERENCE_JP.md)**を
> 開いてください。起動時の61ワードすべてを説明し、最初に「これを使うと何ができるか」を案内します。
> 特に`DEFER ' ['] IS ACTION-OF`の実例では、呼出し元を再compileせず、選んだ処理だけを動的に
> 切り替えられることがわかります。
